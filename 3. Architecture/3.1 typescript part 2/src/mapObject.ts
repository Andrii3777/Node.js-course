...
function mapObject<T, V>(
    obj: Record<K, T>, transformer: (value: T) => V
): Record<K, V> {
...
console.log(mapObject(arr1, (e: number) => e > 2));
console.log(mapObject(arr2, (e: string) => e.length));
console.log(mapObject(arr3, (e: boolean) => e ? 'yes' : 'no'));
